...
RUN
    ...
    CASE("slice")
        StringObject* substr1 = String.slice(s1, mem, 0, 5);
        ...
        substr1 = String.slice(s1, mem, 0, String.size(s1));
    ...
    CASE("NULL slice")
        StringObject* substr = String.slice(s1, mem, -1, 5);
        ASSERT(substr == NULL);
        substr = String.slice(s1, mem, 0, String.size(s1)+1);
        ASSERT(substr == NULL);
        substr = String.slice(s1, mem, 5, 0);
        ASSERT(substr == NULL);
    END
    
    CASE("Empty slice")
        StringObject* substr = String.slice(s1, mem, 0, 0);
        ASSERT(String.size(substr) == 0);
        long size = String.size(s1);
        substr = String.slice(s1, mem, size, size);
        ASSERT(String.size(substr) == 0);
    END
